...
<div class="max-w-7xl mx-auto mb-20 px-4 sm:px-6 lg:px-8">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
        ...
        <div class="lg:col-span-8 space-y-8">
            ...
            <!-- Token Packages Card -->
            <div
                class="bg-white rounded-[2.5rem] p-8 md:p-10 shadow-xl shadow-slate-200/50 border border-slate-100 relative overflow-hidden">
                ...
                <form id="topupForm" class="relative z-10" onsubmit="event.preventDefault(); showPaymentModal();">
                    ...
                    <div class="relative group mb-8">
                        <div
                            class="absolute -inset-0.5 bg-gradient-to-r from-slate-200 to-slate-300 rounded-2xl opacity-50 group-hover:opacity-100 transition duration-300 blur-sm">
                        </div>
                        <div class="relative bg-white p-2 rounded-2xl flex items-center shadow-sm">
                            ...
                            <div
                                class="pr-6 text-xs font-bold text-slate-400 uppercase tracking-widest hidden sm:block">
                                Min. 10 Token
                            </div>
                        </div>
                    </div>
            ...
            <!-- Security Badge -->
            <div
                class="flex flex-wrap justify-center gap-6 opacity-60 grayscale hover:grayscale-0 transition-all duration-500 py-4">
        ...
        <div class="lg:col-span-4 space-y-8">
            <div
                class="bg-white rounded-[2.5rem] p-8 shadow-xl shadow-slate-200/50 border border-slate-100 flex flex-col h-full min-h-[500px]">
                <div class="flex justify-between items-center mb-6">
                    <h3 class="font-bold text-lg text-slate-900">Riwayat Transaksi</h3>
                    <a href="index.php?page=topup_history"
                        class="p-2 bg-slate-50 text-slate-500 hover:text-primary rounded-xl transition-colors">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                            <path fill-rule="evenodd"
                ...
                <div class="flex-grow overflow-y-auto space-y-3 pr-2 custom-scrollbar">
                    ...
                    <?php else: ?>
                        <?php foreach ($history as $h): ?>
                            ...
                            ?>
                            <div
                                class="group relative bg-white border border-slate-100 rounded-2xl p-4 hover:border-slate-300 transition-colors cursor-default">
                                <div class="flex justify-between items-start mb-2">
                                    <div>
                                        <p class="text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-0.5">Top Up
                                            Token</p>
                                    ...
                                    <div
                                        class="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wide border <?= $statusStyle ?>">
                                        <?= $statusIcon ?>
                                        <?= $h['status'] ?>
                                    </div>
                                </div>
                                <div class="flex justify-between items-center pt-2 border-t border-slate-50">
                                    <span
                                        class="text-[10px] font-mono text-slate-400 font-bold">#<?= str_pad($h['id'], 8, '0', STR_PAD_LEFT) ?></span>
                                    <span
                                        class="text-[10px] font-bold text-slate-400 bg-slate-50 px-2 py-0.5 rounded"><?= date('d M, H:i', strtotime($h['created_at'])) ?></span>
                                </div>
                            </div>
                        <?php endforeach; ?>
                    <?php endif; ?>
...
<div id="paymentModal" class="fixed inset-0 z-[100] hidden overflow-hidden" role="dialog" aria-modal="true">
    <div class="absolute inset-0 bg-slate-900/40 backdrop-blur-sm transition-opacity" onclick="closePaymentModal()">
    </div>
        ...
        <div class="flex-grow bg-white flex flex-col relative">
            ...
            <div class="h-20 border-b border-slate-100 flex items-center justify-between px-8 bg-white z-20">
                ...
                <button onclick="closePaymentModal()"
                    class="w-10 h-10 rounded-full bg-slate-50 text-slate-400 hover:text-rose-500 hover:bg-rose-50 flex items-center justify-center transition-all">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="flex-grow p-8 md:p-12 overflow-y-auto bg-slate-50/30" id="paymentDetails">
                ...
                <div class="h-full flex flex-col items-center justify-center text-center opacity-50 select-none">
                    ...
                    <p class="text-sm text-slate-400 max-w-xs mx-auto">Silakan pilih metode pembayaran yang tersedia di
                        menu sebelah kiri untuk melanjutkan transaksi Anda.</p>
                </div>
            </div>

            <!-- Bottom Action Bar -->
            <div class="p-6 border-t border-slate-100 bg-white z-30 hidden shadow-[0_-5px_20px_rgba(0,0,0,0.02)]"
                id="paymentAction">
                <div class="flex items-center gap-4">
                    ...
                    <button onclick="confirmPayment()"
                        class="flex-grow md:flex-grow-0 md:w-auto w-full px-8 py-4 bg-slate-900 text-white font-bold rounded-xl shadow-lg shadow-slate-900/10 hover:bg-primary hover:shadow-primary/30 transition-all flex items-center justify-center gap-3">
                    ...
                    <button onclick="closePaymentModal()"
                        class="hidden md:flex px-6 py-4 border border-slate-200 text-slate-500 font-bold rounded-xl hover:bg-slate-50 hover:text-slate-800 transition-all">
                        Batal
                    </button>
                </div>
            </div>
